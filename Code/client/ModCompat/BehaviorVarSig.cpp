...
void BehaviorVarSig::initialize()
{
    ...
    for (auto& item : dirs)
    {
        ...
        {
            auto add = loadAddFromDir(item);
            if (add)
            {
                addPool.push_back(std::move(add));
            }
        }
    }
}
...
TiltedPhoques::UniquePtr<BehaviorVarSig::Add> BehaviorVarSig::loadAddFromDir(std::string aDir)
{
    ...
    // create the add
    TiltedPhoques::UniquePtr<Add> result(new Add());
    result->mHash = hash;
...
void BehaviorVarSig::patchAdd(BehaviorVarSig::Add& aAdd)
{
    ...
    // new AnimationGraphDescriptorManager::Builder(AnimationGraphDescriptorManager::Get(),
    // aAdd.mHash,*animGrapDescriptor);
}
