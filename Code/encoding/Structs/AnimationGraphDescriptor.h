...
struct AnimationGraphDescriptor
{
    ...
    // Add new constructor overload to allow initialization with dynamic data
    AnimationGraphDescriptor(const TiltedPhoques::Vector<uint32_t>& booleanVars,
                             const TiltedPhoques::Vector<uint32_t>& floatVars,
                             const TiltedPhoques::Vector<uint32_t>& integerVars)
        : BooleanLookUpTable(booleanVars), FloatLookupTable(floatVars), IntegerLookupTable(integerVars)
    {
    }
